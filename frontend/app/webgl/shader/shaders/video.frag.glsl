...
// Helper: Apply vignette effect
float vignette(vec2 uv, float intensity, float radius, float smoothness) {
  // Calculate distance from center
  vec2 center = vec2(0.5, 0.5);
  float dist = distance(uv, center);

  // Apply vignette with smoothstep
...
void main() {
  ...
  // Apply vignette if intensity > 0
  if (u_vignetteIntensity > 0.01) {
    float vignetteMultiplier = vignette(v_texCoord, u_vignetteIntensity, u_vignetteRadius, u_vignetteSmoothness);
